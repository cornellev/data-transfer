...
Generated from: ('sensor_data.xml',)
...
MAVLINK_MSG_ID_SENSOR_DATA = 200


class MAVLink_sensor_data_message(MAVLink_message):
    """
    Custom sensor data message
    """

    id = MAVLINK_MSG_ID_SENSOR_DATA
    msgname = "SENSOR_DATA"
    fieldnames = ["gps_lat", "gps_long", "left_rpm", "right_rpm", "x_accel", "y_accel", "z_accel", "temp"]
    ordered_fieldnames = ["gps_lat", "gps_long", "x_accel", "y_accel", "z_accel", "temp", "left_rpm", "right_rpm"]
    fieldtypes = ["double", "double", "int32_t", "int32_t", "double", "double", "double", "double"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<ddddddii")
    orders = [0, 1, 6, 7, 2, 3, 4, 5]
    lengths = [1, 1, 1, 1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0, 0, 0, 0]
    crc_extra = 73
    unpacker = struct.Struct("<ddddddii")
    instance_field = None
    instance_offset = -1

    def __init__(self, gps_lat: float, gps_long: float, left_rpm: int, right_rpm: int, x_accel: float, y_accel: float, z_accel: float, temp: float):
        MAVLink_message.__init__(self, MAVLink_sensor_data_message.id, MAVLink_sensor_data_message.msgname)
        self._fieldnames = MAVLink_sensor_data_message.fieldnames
        self._instance_field = MAVLink_sensor_data_message.instance_field
        self._instance_offset = MAVLink_sensor_data_message.instance_offset
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.gps_lat, self.gps_long, self.x_accel, self.y_accel, self.z_accel, self.temp, self.left_rpm, self.right_rpm), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_sensor_data_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_SENSOR_DATA: MAVLink_sensor_data_message,
}
...
class MAVLink(object):
    ...
    def sensor_data_encode(self, gps_lat: float, gps_long: float, left_rpm: int, right_rpm: int, x_accel: float, y_accel: float, z_accel: float, temp: float) -> MAVLink_sensor_data_message:
        """
        Custom sensor data message

        gps_lat                   : Latitude (deg) (type:double)
        gps_long                  : Longitude (deg) (type:double)
        left_rpm                  : Left motor RPM (type:int32_t)
        right_rpm                 : Right motor RPM (type:int32_t)
        x_accel                   : X acceleration (m/s^2) (type:double)
        y_accel                   : Y acceleration (m/s^2) (type:double)
        z_accel                   : Z acceleration (m/s^2) (type:double)
        temp                      : Temperature (deg C) (type:double)

        """
        return MAVLink_sensor_data_message(gps_lat, gps_long, left_rpm, right_rpm, x_accel, y_accel, z_accel, temp)

    def sensor_data_send(self, gps_lat: float, gps_long: float, left_rpm: int, right_rpm: int, x_accel: float, y_accel: float, z_accel: float, temp: float, force_mavlink1: bool = False) -> None:
        """
        Custom sensor data message
        ...
        """
        self.send(self.sensor_data_encode(gps_lat, gps_long, left_rpm, right_rpm, x_accel, y_accel, z_accel, temp), force_mavlink1=force_mavlink1)
